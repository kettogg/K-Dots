...
K_PLUGIN_FACTORY_WITH_JSON(BreezeDecoFactory, "breeze.json", registerPlugin<Breeze::Decoration>();
...
//________________________________________________________________
Decoration::Decoration(QObject *parent, const QVariantList &args) : KDecoration2::Decoration(parent, args), m_animation(new QVariantAnimation(this)) {
    g_sDecoCount++;
}
...
QColor Decoration::rawTitleBarColor() const {
    ...
    if (!matchColorForTitleBar()) {
        if (m_animation->state() == QAbstractAnimation::Running) {
            titleBarColor =
                KColorUtils::mix(c->color(ColorGroup::Inactive, ColorRole::TitleBar), c->color(ColorGroup::Active, ColorRole::TitleBar), m_opacity);
        } else
...
void Decoration::paint(QPainter *painter, const QRect &repaintRegion) {
    ...
    if (!c->isShaded()) {
        ...
        QPen border_pen1(QColor(154, 154, 155));

        if (qGray(titleBarColor.rgb()) < 128) {
            border_pen1.setColor(QColor(50, 50, 50));
        }
        if (c->isActive()) {
            border_pen1.setColor(QColor(0, 0, 0, 0));
        }
        if (isMaximized()) {
            border_pen1.setColor(titleBarColor);
        }
        if (isMaximizedVertically()) {
            border_pen1.setColor(titleBarColor.darker(125));
        }
        border_pen1.setWidth(1);
    ...
    if (hasBorders()) {
        ...
        if (c->isActive()) {
            border_pen1.setColor(QColor(0, 0, 0));
        }
        if (isMaximized()) {
            border_pen1.setColor(titleBarColor);
        }
        ...
        if (s->isAlphaChannelSupported()) {
            if (isLeftEdge())
                painter->drawRoundedRect(rect().adjusted(-1, 0, 0, 0), m_internalSettings->cornerRadius(), m_internalSettings->cornerRadius());
            else if (isRightEdge())
                painter->drawRoundedRect(rect().adjusted(0, 0, 1, 0), m_internalSettings->cornerRadius(), m_internalSettings->cornerRadius());
            else
                painter->drawRoundedRect(rect(), m_internalSettings->cornerRadius(), m_internalSettings->cornerRadius());
        } else
            painter->drawRect(rect());
...
void Decoration::paintTitleBar(QPainter *painter, const QRect &repaintRegion) {
    ...
    if (!s->isAlphaChannelSupported())
        ...
        painter->drawRoundedRect(
            titleRect.adjusted(isLeftEdge() ? -m_internalSettings->cornerRadius() : 0, isTopEdge() ? -m_internalSettings->cornerRadius() : 0,
                               isRightEdge() ? m_internalSettings->cornerRadius() : 0, m_internalSettings->cornerRadius()),
    ...
    if (!hideTitleBar()) {
        // fix bottom background rounded color
        // TODO: Fix static value
        painter->setPen(Qt::NoPen);
        painter->setBrush(titleBarColor);
        painter->drawRect(titleRect.adjusted(0, 12, 0, 0));
